...
// Saytga kiring, F12 > Console'ga yopishtiring va Enter bosing

(async function() {
  // Supabase client'ni saytdan olish
  // Agar saytda supabase client mavjud bo'lsa, uni ishlatamiz
  let supabase;
  
  try {
    // React DevTools orqali supabase client'ni olish
    // Yoki to'g'ridan-to'g'ri import qilish
    const { createClient } = await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm');
    
    // Environment variables'larni olish
    ...
    const SUPABASE_KEY = window.__ENV__?.VITE_SUPABASE_PUBLISHABLE_KEY || 
    ...
    console.log('📝 Iltimos, saytning .env faylidan VITE_SUPABASE_URL va VITE_SUPABASE_PUBLISHABLE_KEY ni oling');
    return;
  }
